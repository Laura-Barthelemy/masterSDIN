...
with open ('DataNetworkSDIN','rb') as base:
    A=pickle.load(base)
...
# Attributs des noeuds
        ...
        and Dict_degree_centrality[noeud]<0.25:
            g.node[noeud]={'size':100,'color':'yellow'}
            
        elif Dict_degree_centrality[noeud]>=0.25\
        and Dict_degree_centrality[noeud]<0.35:
            g.node[noeud]={'size':500,'color':'orange'\
            ,'name':Information[noeud]['name']}
            
        elif Dict_degree_centrality[noeud]>=0.35\
        and Dict_degree_centrality[noeud]<0.5:
            g.node[noeud]={'size':1000,'color':'red'\
            ,'name':Information[noeud]['name']}
            
        else:
            g.node[noeud]={'size':1500,'color':'blue'\
            ,'name':Information[noeud]['name']}


## Sélection des noeuds + attributs de taille, couleur et nom (Suite)
Node_Color=[]
Node_Size=[]
Node_Label={}
...
# lien pour 2 noeuds moins de 25% du réseau
...
nx.draw_networkx_nodes(g, pos=p, node_size=Node_Size,node_color=Node_Color\
,alpha=1)
...
plt.title('Communauté du Master SDIN', size=50)
plt.axis('off')
plt.savefig('communaute_SDIN.pdf', format='pdf')
